...
export default function ComboBuilderPage() {
  ...
  // Update box size slots count
  const handleBoxSizeChange = (size: number) => {
    setBoxSize(size);
    setSlots(new Array(size).fill(null));
    setActiveSlotIdx(0);
  };
  ...
  const handleProceedToCheckout = () => {
    ...
    const comboName = `${boxSize}-Bottle Discovery Box (Custom Bundle)`;
      ...
      <main className="flex-1 bg-white py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            <span className="text-[10px] font-bold tracking-[0.3em] text-primary uppercase font-mono">MOLECULAR STACKING</span>
            <h1 className="font-sans text-3xl sm:text-5xl font-black tracking-tight text-slate-900 mt-2">
              SCENT COMBO MATRIX BUILDER
            </h1>
            <p className="text-slate-500 text-xs sm:text-sm font-light tracking-wide max-w-md mx-auto mt-3">
              Configure personal decants, assign fluid volumes, and compile your own custom discovery matrix.
            </p>
          </div>
            ...
            <div className="lg:col-span-7 bg-slate-50 border border-slate-200 rounded-2xl p-6 sm:p-8 space-y-8">
                ...
                <div className="flex gap-4">
                  {[3, 5].map((size) => (
                    <button
                      ...
                      className={`flex-1 rounded-xl py-3.5 text-xs font-bold tracking-widest uppercase transition ${
                        boxSize === size
                          ? 'bg-primary text-white shadow-md shadow-blue-500/10'
                          : 'bg-white border border-slate-200 text-slate-650 hover:bg-slate-100 hover:border-slate-350'
                      }`}
                    >
                      {size}-Slot discovery Box
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <h3 className="block text-[10px] font-bold tracking-wider text-slate-400 font-mono uppercase mb-3">
                  2. FLUID SIZE SETTINGS (FOR NEW SLOTS)
                </h3>
                    ...
                    >
                        ...
                        {slot ? (
                          <div>
                            ...
                            <div className="text-[10px] text-slate-500 font-mono mt-0.5">
                              {slot.selectedSizeMl}ml extract • Batch Code: {slot.internalFormulaKey}
                            </div>
                          </div>
                        ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {!slot && activeSlotIdx === idx && (
                          <span className="text-[9px] font-bold text-blue-500 tracking-wider font-mono">ACTIVE SELECT</span>
                        )}
                      </div>
                    </motion.div>
                  ))}
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                ...
                {perfumes.length === 0 ? (
                  <div className="text-slate-400 text-xs py-8 text-center flex flex-col items-center justify-center">
                    ...
                    <span>No synthesised scents in registry.</span>
                  </div>
                ) : (
                ...
                <div className="flex items-start gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 leading-relaxed">
                      Your custom discovery deck items will be compiled under a single box price. Fill all slots to checkout.
                    </p>
                  </div>
                </div>
